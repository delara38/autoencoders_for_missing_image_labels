...
if __name__ == '__main__':
    ...
    trainloader = torch.utils.data.DataLoader(obs, batch_size=batch_size,
                                            shuffle=False)
    tot_loader = torch.utils.data.DataLoader(p, batch_size=batch_size,
                                            shuffle=False)
def train_autoencoder(epochs=5):
  ...
  for epoch in tqdm(range(epochs)):
    ...
    running_loss = 0.0
    for i, data in enumerate(tot_loader):
...
if __name__ == '__main__':
    ...
    cc = train_classifier(40)

    na, decoder = train_autoencoder(40)
    ...
    torch.save(na, "net_autoencoder.pt")
    torch.save(cc, 'net_classifier.pt')


    import pickle

    pickle.dump({'missing':missing,'obs':obs, 'pa':pa, 'pc':pc, 'labels':labels}, open('pmm_data.pkl','wb'))