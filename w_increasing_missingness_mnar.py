...
if __name__ == '__main__':
    ...
    beta_params = [(1,1), (10, 10), (50, 50)]
    
    
    for beta_param in beta_params:
        print(beta_param)
        missingness_dict = [np.random.beta(beta_param[0], beta_param[1]) for i in range(10)]
        missing = []
        obs = []
        for img in p:
        ...
        torch.save(na, f"net_autoencoder_beta_{beta_param[0]}_{beta_param[1]}.pt")
        torch.save(cc, f'net_classifier_beta_{beta_param[0]}_{beta_param[1]}.pt')


        import pickle

        pickle.dump({'missing':missing,'obs':obs, 'pa':pa, 'pc':pc, 'labels':labels,'missingness_dict':missingness_dict}, open(f'pmm_data_betas_{beta_param[0]}_{beta_param[1]}.pkl','wb'))